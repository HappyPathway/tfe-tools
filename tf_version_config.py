...
def get_version(file):
    with open(file) as tf_input:
        data = hcl2.loads(tf_input.read())

    if 'terraform' in data.keys():
        for config_item in data.get('terraform'):
            if 'required_version' in config_item:
                return config_item.get('required_version')
    return None
...
def list_files(ws_name):
    configured_version = False
    for tf_file_name in glob("*.tf"):
        version = get_version(tf_file_name)
        if version:
            configured_version = True
...
def main(terraform_url, terraform_org, github_base, basedir, git_namespace, workspace):
    ...
    version_dict = list_files(ws.name, config=True)
    shutil.rmtree(basedir)
    print(json.dumps(version_dict, sort_keys=True, indent=4, separators=(',', ':')))
